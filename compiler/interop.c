...
#define DMG_READ_PAGE_OFFSET 0
#define DMG_WRITE_PAGE_OFFSET (256 * 4)
...
static void emit_slow_dmg_write(struct code_block *block, uint8_t val_reg)
{
...
static void compile_inline_dmg_write(struct code_block *block, uint8_t val_reg)
{
    ...
    // move.b val_reg, (a0,d0.w)         ; 4 bytes [30-33]
    emit_move_b_dn_idx_an(block, val_reg, REG_68K_A_SCRATCH_1, REG_68K_D_SCRATCH_0);
    // bra.s done (+48)                  ; 2 bytes [34-35] -> offset 84
    emit_bra_b(block, 48);

    // check_hram: (offset 36)
    // cmpi.w #$ff80, d1                 ; 4 bytes [36-39]
    emit_cmpi_w_imm_dn(block, 0xff80, REG_68K_D_SCRATCH_1);
    // bcs.s slow_path (+22)             ; 2 bytes [40-41] -> offset 64
    emit_bcs_b(block, 22);
    // cmpi.w #$ffff, d1                 ; 4 bytes [42-45]
    emit_cmpi_w_imm_dn(block, 0xffff, REG_68K_D_SCRATCH_1);
    // beq.s slow_path (+16)             ; 2 bytes [46-47] -> offset 64
    emit_beq_b(block, 16);

    // hram access (offset 48):
    // movea.l JIT_CTX_HRAM_BASE(a4), a0 ; 4 bytes [48-51]
    emit_movea_l_disp_an_an(block, JIT_CTX_HRAM_BASE, REG_68K_A_CTX, REG_68K_A_SCRATCH_1);
    // move.w d1, d0                     ; 2 bytes [52-53]
    emit_move_w_dn_dn(block, REG_68K_D_SCRATCH_1, REG_68K_D_SCRATCH_0);
    // subi.w #$ff80, d0                 ; 4 bytes [54-57]
    emit_subi_w_dn(block, 0xff80, REG_68K_D_SCRATCH_0);
    // move.b val_reg, (a0,d0.w)         ; 4 bytes [58-61]
    emit_move_b_dn_idx_an(block, val_reg, REG_68K_A_SCRATCH_1, REG_68K_D_SCRATCH_0);
    // bra.s done (+20)                  ; 2 bytes [62-63] -> offset 84
    emit_bra_b(block, 20);
...
static void emit_slow_dmg_read(struct code_block *block)
{
...
void compile_call_dmg_read_to_d0(struct code_block *block)
{
    ...
    // movea.l JIT_CTX_DMG(a4), a0       ; 4 bytes [0-3]
    emit_movea_l_disp_an_an(block, JIT_CTX_DMG, REG_68K_A_CTX, REG_68K_A_SCRATCH_1);
    // move.w d1, d0                     ; 2 bytes [4-5]
    emit_move_w_dn_dn(block, REG_68K_D_SCRATCH_1, REG_68K_D_SCRATCH_0);
    // lsr.w #8, d0                      ; 2 bytes [6-7]
    emit_lsr_w_imm_dn(block, 8, REG_68K_D_SCRATCH_0);
    // lsl.w #2, d0                      ; 2 bytes [8-9]
    emit_lsl_w_imm_dn(block, 2, REG_68K_D_SCRATCH_0);
    // movea.l (a0,d0.w), a0  ; 4 bytes [10-13]
    emit_movea_l_idx_an_an(block, 0, REG_68K_A_SCRATCH_1, REG_68K_D_SCRATCH_0, REG_68K_A_SCRATCH_1);
    // cmpa.w #0, a0                     ; 4 bytes [14-17]
    emit_cmpa_w_imm_an(block, 0, REG_68K_A_SCRATCH_1);
    // beq.s check_hram (+12)            ; 2 bytes [18-19] -> offset 32
    emit_beq_b(block, 12);

    // Page hit (offset 20):
    // move.w d1, d0                     ; 2 bytes [20-21]
    emit_move_w_dn_dn(block, REG_68K_D_SCRATCH_1, REG_68K_D_SCRATCH_0);
    // andi.w #$ff, d0                   ; 4 bytes [22-25]
    emit_andi_w_dn(block, REG_68K_D_SCRATCH_0, 0x00ff);
    // move.b (a0,d0.w), d0              ; 4 bytes [26-29]
    emit_move_b_idx_an_dn(block, REG_68K_A_SCRATCH_1, REG_68K_D_SCRATCH_0, REG_68K_D_SCRATCH_0);
    // bra.s done (+42)                  ; 2 bytes [30-31] -> offset 74
    emit_bra_b(block, 42);

    // check_hram: (offset 32)
    // cmpi.w #$ff80, d1                 ; 4 bytes [32-35]
    emit_cmpi_w_imm_dn(block, 0xff80, REG_68K_D_SCRATCH_1);
    // bcs.s slow_path (+22)             ; 2 bytes [36-37] -> offset 60
    emit_bcs_b(block, 22);
    // cmpi.w #$ffff, d1                 ; 4 bytes [38-41]
    emit_cmpi_w_imm_dn(block, 0xffff, REG_68K_D_SCRATCH_1);
    // beq.s slow_path (+16)             ; 2 bytes [42-43] -> offset 60
    emit_beq_b(block, 16);

    // hram access (offset 44):
    // movea.l JIT_CTX_HRAM_BASE(a4), a0 ; 4 bytes [44-47]
    emit_movea_l_disp_an_an(block, JIT_CTX_HRAM_BASE, REG_68K_A_CTX, REG_68K_A_SCRATCH_1);
    // move.w d1, d0                     ; 2 bytes [48-49]
    emit_move_w_dn_dn(block, REG_68K_D_SCRATCH_1, REG_68K_D_SCRATCH_0);
    // subi.w #$ff80, d0                 ; 4 bytes [50-53]
    emit_subi_w_dn(block, 0xff80, REG_68K_D_SCRATCH_0);
    // move.b (a0,d0.w), d0              ; 4 bytes [54-57]
    emit_move_b_idx_an_dn(block, REG_68K_A_SCRATCH_1, REG_68K_D_SCRATCH_0, REG_68K_D_SCRATCH_0);
    // bra.s done (+14)                  ; 2 bytes [58-59] -> offset 74
    emit_bra_b(block, 14);

    // slow_path: (offset 60)
    emit_slow_dmg_read(block);
    // falls through to done (offset 74)
}
