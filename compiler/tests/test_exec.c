...
TEST(test_exec_cp_equal)
{
    ...
    ASSERT_EQ(get_dreg(REG_68K_D_FLAGS) & 0x04, 0x04);  // Z flag set (bit 7)
}
...
TEST(test_exec_cp_carry)
{
    ...
    run_program(rom, 0);
    ASSERT_EQ(get_dreg(REG_68K_D_FLAGS) & 1, 1);  // C flag set (bit 4)
}
...
// Pokemon TryDoWildEncounter pattern tests:
TEST(test_chained_ret_nz_both_return)
{
    ...
    run_program(rom, 0);
    // If chained ret nz works: A = 0xaa
    // If middle's ret nz fails to see Z=0: A = 0xcc
    ASSERT_EQ(get_dreg(REG_68K_D_A) & 0xff, 0xaa);
}
...
TEST(test_chained_ret_nz_inner_falls_through)
{
    ...
    run_program(rom, 0);
    // Middle's ret nz should see Z=0 and return
    ASSERT_EQ(get_dreg(REG_68K_D_A) & 0xff, 0xaa);
}
