...
// move.b #imm, Dn
static void emit_move_b_dn(struct basic_block *block, uint8_t reg, int8_t imm)
{
    // 01 = byte mode
...
// rol.w #8, Dn
static void emit_rol_w_8(struct basic_block *block, uint8_t reg)
{
...
// ror.w #8, Dn
static void emit_ror_w_8(struct basic_block *block, uint8_t reg)
{
...
static void emit_swap(struct basic_block *block, uint8_t reg)
{
...
static void emit_move_w_an_dn(struct basic_block *block, uint8_t areg, uint8_t dreg)
{
...
static void emit_movea_w_dn_an(struct basic_block *block, uint8_t dreg, uint8_t areg)
{
...
struct basic_block *compile_block(uint16_t src_address, uint8_t *gb_code)
{
    ...
    while (!done) {
        ...
        case 0x26: // ld h, imm8 - HL is in A0 (contiguous 0xHHLL)
        ...
        case 0x2e: // ld l, imm8 - HL is in A0 (contiguous 0xHHLL)
            emit_move_w_an_dn(block, 0, 3);      // move.w a0, d3
            emit_move_b_dn(block, 3, gb_code[src_ptr++]);
            emit_movea_w_dn_an(block, 3, 0);     // movea.w d3, a0
            break;
